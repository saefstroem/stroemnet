...
impl PeerDb {
    pub fn get_swaps_for_channel(&self, channel: ChannelId) -> Result<Vec<([u8; 32], Vec<u8>)>> {
        ...
        for row in range {
            let (k, v) = row?;
            let key = k.value();
            if let Some((_prefix, rest)) = key.split_first()
                && let Ok(swap_id) = <[u8; 32]>::try_from(rest)
            {
...
mod tests {
    ...
    fn swap_set_get_delete_roundtrip() {
        ...
        db.set_swap(ChannelId::IgraGalleon, &id, &[1, 2, 3])
            .unwrap();
        assert_eq!(
    ...
    fn load_channel_isolates_by_channel() {
        let dir = tempdir().unwrap();
        let db = PeerDb::new(&dir.path().join("s.db")).unwrap();
        db.set_swap(ChannelId::IgraGalleon, &[1u8; 32], &[0xaa])
            .unwrap();
        db.set_swap(ChannelId::KaspaTn10, &[2u8; 32], &[0xbb])
            .unwrap();
        assert_eq!(
            db.get_swaps_for_channel(ChannelId::IgraGalleon).unwrap(),
            vec![([1u8; 32], vec![0xaa])]
